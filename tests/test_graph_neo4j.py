...
def test_graph_db_service_is_healthy(graph_db_url):
    ...
    response = httpx.get(f"{graph_db_url}/health")
...
@pytest.mark.integration
@pytest.mark.requires_docker
def test_upsert_author_via_api(graph_db_url, sample_authors):
    ...
    response = httpx.post(f"{graph_db_url}/authors", json=payload)
    assert response.status_code == 200
...
@pytest.mark.integration
@pytest.mark.requires_docker
def test_upsert_work_via_api(graph_db_url, sample_works):
    ...
    response = httpx.post(f"{graph_db_url}/works", json=payload)
    assert response.status_code == 200
...
@pytest.mark.integration
@pytest.mark.requires_docker
def test_link_author_work_via_api(graph_db_url, sample_authors, sample_works):
...
@pytest.mark.integration
@pytest.mark.requires_docker
def test_collaborator_discovery_via_api(graph_db_url, sample_authors, sample_works):
...
@pytest.mark.integration
@pytest.mark.requires_docker
def test_viz_network_via_api(graph_db_url, sample_authors, sample_works):
