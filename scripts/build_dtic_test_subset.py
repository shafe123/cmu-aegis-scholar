...
def build_work_meta(work: dict, authors_map: dict[str, dict], orgs_map: dict[str, dict], topics_map: dict[str, dict]) -> WorkMeta:
    ...
    author_ids = unique_preserve_order(entry.get("author_id") for entry in author_entries if isinstance(entry, dict))
    author_orgs = unique_preserve_order(entry.get("org_id") for entry in author_entries if isinstance(entry, dict))
    work_orgs = unique_preserve_order(entry.get("org_id") for entry in work_org_entries if isinstance(entry, dict))
    topic_ids = unique_preserve_order(entry.get("topic_id") for entry in work_topic_entries if isinstance(entry, dict))
    ...
    return WorkMeta(
        ...
        has_author_missing_org=any(not (entry.get("org_id") or "").strip() for entry in author_entries if isinstance(entry, dict)),
        all_authors_present=all(author_id in authors_map for author_id in author_ids),
        all_orgs_present=all(org_id in orgs_map for org_id in set(author_orgs).union(work_orgs)),
...
def can_add_work(meta: WorkMeta, seen_authors: set[str], seen_orgs: set[str], seen_topics: set[str], max_authors: int, max_orgs: int, max_topics: int) -> bool:
    ...
    return projected_authors <= max_authors and projected_orgs <= max_orgs and projected_topics <= max_topics
...
def choose_work_batch(
    ...
    picks: list[WorkMeta] = []
    remaining = [meta for meta in works if meta.id not in selected_ids and predicate(meta)]
    ...
    while remaining and len(picks) < quota:
        ...
        for meta in remaining:
            if can_add_work(meta, seen_authors, seen_orgs, seen_topics, max_authors, max_orgs, max_topics):
                chosen = meta
                break
        if chosen is None:
            break
...
def choose_extra_authors(
    ...
    def add_from(predicate: Callable[[dict], bool], max_take: int) -> None:
        ...
        for author_id, author in authors_map.items():
            ...
            candidates.append((new_orgs, len(orgs), author.get("works_count", 0), author.get("name", ""), author_id))
    ...
    add_from(lambda a: (a.get("citation_count") or 0) == 0 and (a.get("works_count") or 0) > 0, 4)
...
def fill_entities(required_ids: set[str], entity_map: dict[str, dict], target_count: int, sort_key: Callable[[dict], tuple]) -> list[dict]:
    """Return exactly target_count entities, starting from required linked ones."""
    selected = [entity_map[entity_id] for entity_id in required_ids if entity_id in entity_map]
    ...
    extras = [obj for entity_id, obj in entity_map.items() if entity_id not in selected_ids]
...
def analyze_subset(authors: list[dict], works: list[dict], orgs: list[dict], topics: list[dict]) -> dict:
        ...
        for author_entry in work.get("authors") or []:
            ...
            if author_id:
                if author_id in author_ids:
                    ...
                    work_author_missing.append({"work_id": work["id"], "author_id": author_id})

            org_id = author_entry.get("org_id")
            if org_id and org_id not in org_ids:
                work_org_missing.append({"work_id": work["id"], "org_id": org_id, "source": "work.authors"})

        for org_entry in work.get("orgs") or []:
            org_id = org_entry.get("org_id")
            if org_id and org_id not in org_ids:
                work_org_missing.append({"work_id": work["id"], "org_id": org_id, "source": "work.orgs"})
    ...
    analysis = {
        ...
        "integrity": {
            ...
            "is_fully_linked": not any([work_author_missing, work_org_missing, work_topic_missing, author_org_missing]),
        },
        "connectivity": {
            "authors_with_selected_works": sum(1 for author in authors if linked_work_counts.get(author["id"], 0) > 0),
            "authors_without_selected_works": sum(1 for author in authors if linked_work_counts.get(author["id"], 0) == 0),
            "orgs_referenced_by_subset": sum(
                1
                for org in orgs
                if any(org["id"] in (author.get("org_ids") or []) for author in authors)
                or any(org["id"] == org_entry.get("org_id") for work in works for org_entry in (work.get("orgs") or []))
            ),
            "topics_referenced_by_subset": sum(
                1 for topic in topics if any(topic["id"] == topic_entry.get("topic_id") for work in works for topic_entry in (work.get("topics") or []))
            ),
        },
        "edge_cases": {
            "authors_no_orgs": sum(1 for author in authors if not (author.get("org_ids") or [])),
            "authors_zero_works": sum(1 for author in authors if (author.get("works_count") or 0) == 0),
            "authors_zero_citations": sum(1 for author in authors if (author.get("citation_count") or 0) == 0),
            "authors_special_names": sum(1 for author in authors if has_special_name(author.get("name"))),
            "works_missing_abstract": sum(1 for work in works if not (work.get("abstract") or "").strip()),
            "works_missing_doi": sum(1 for work in works if not (work.get("doi") or "").strip()),
            "works_no_authors": sum(1 for work in works if not (work.get("authors") or [])),
            "works_author_missing_org": sum(
                1
                for work in works
                if any(not (entry.get("org_id") or "").strip() for entry in (work.get("authors") or []))
            ),
            "works_no_topics": sum(1 for work in works if not (work.get("topics") or [])),
...
def main() -> int:
    ...
    parser.add_argument("--input-dir", default="data/dtic_compressed", help="Directory containing the source DTIC JSONL files")
    parser.add_argument("--output-dir", default="tests/dtic_test_subset", help="Directory where the linked subset should be written")
    parser.add_argument("--target-count", type=int, default=50, help="Exact number of authors/works/orgs/topics to output")
    ...
    works = [build_work_meta(obj, authors_map, orgs_map, topics_map) for obj in iter_jsonl(input_dir / "dtic_works_001.jsonl") if obj.get("id")]

    work_categories: OrderedDict[str, tuple[int, Callable[[WorkMeta], bool]]] = OrderedDict(
        [
            (
                "baseline",
                (
                    20,
                    lambda meta: meta.has_abstract
                    and meta.has_doi
            ...
            (
                ...
                (
                    8,
                    lambda meta: (not meta.has_abstract)
                    and meta.has_authors
                    and meta.has_topics
                    and meta.has_orgs
                    and meta.all_authors_present
            ...
            (
                ...
                (
                    5,
                    lambda meta: (not meta.has_authors)
                    and meta.has_abstract
                    and meta.has_topics
                    and meta.has_orgs
                    and meta.all_orgs_present,
    ...
    for category_name, (quota, predicate) in work_categories.items():
        ...
        if len(batch) < quota:
            raise ValueError(f"Could not satisfy work category '{category_name}' with quota={quota}; only found {len(batch)} linked candidates.")
        selected_work_meta.extend(batch)
        selection_breakdown[category_name] = len(batch)

    if len(selected_work_meta) != target:
        raise ValueError(f"Expected {target} works but selected {len(selected_work_meta)}")

    referenced_author_ids = unique_preserve_order(author_id for meta in selected_work_meta for author_id in meta.author_ids if author_id in authors_map)
    ...
    if len(selected_author_ids) != target:
        raise ValueError(f"Expected {target} authors but selected {len(selected_author_ids)}")
    ...
    selected_authors = [authors_map[author_id] for author_id in unique_preserve_order(list(referenced_author_ids) + extra_author_ids)]
    selected_works = [meta.raw for meta in selected_work_meta]

    analysis = analyze_subset(selected_authors, selected_works, selected_orgs, selected_topics)
