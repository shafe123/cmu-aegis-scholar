...
class DTICOrgExtractor:
    ...
    def process_blob(self, blob_name: str) -> int:
        ...
        try:
            ...
            for org_data in raw_work.get('organizations', []):
                ...
                if not grid_id:
                    logger.debug(f"Skipping org without grid_id: {org_data.get('name', 'unknown')}")
                    continue
                ...
                try:
                    ...
                    existing_blob = self.dest_container_client.get_blob_client(org_blob_name)
                ...
                # Upload the org file
                org_blob_client = self.dest_container_client.get_blob_client(org_blob_name)
                org_json = json.dumps(cleaned_org, indent=2, ensure_ascii=False)
                org_blob_client.upload_blob(org_json, overwrite=True)
                
                orgs_found += 1
            
            logger.info(f"[OK] Processed {blob_name}: extracted {orgs_found} organizations")
            return orgs_found
    ...
    def process_all(self, max_files: Optional[int] = None) -> Dict[str, int]:
        ...
        """
        logger.info(f"Starting to extract organizations from {self.source_container}/{self.source_prefix}")
        ...
        logger.info(f"Processing complete: {processed} processed, {failed} failed, {total_orgs_extracted} orgs extracted")
...
def main():
    ...
    try:
        ...
        logger.info(f"This run: {results['total_orgs_extracted']} org instances extracted")
