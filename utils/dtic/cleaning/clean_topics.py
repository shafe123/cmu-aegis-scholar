...
class ExtractionStateManager:
    ...
    def get_or_create_topic_id(self, topic_name: str) -> str:
        ...
        if normalized_name not in self.state['topics']:
            ...
            namespace = uuid.UUID('00000000-0000-0000-0000-000000000003')  # Different namespace for topics
...
class DTICTopicExtractor:
    ...
    def extract_and_clean_topic(self, topic_name: str, topic_data: Optional[Dict] = None) -> Optional[Dict]:
        """
        ...
        topic_entity = {
            ...
            'sources': [{
                'source': 'dtic',
                'id': topic_data.get('id', f"dtic:{self.sanitize_filename(topic_name)}") if topic_data else f"dtic:{self.sanitize_filename(topic_name)}"
            }],
        ...
        try:
            ...
            for keyword in keywords:
                ...
                if isinstance(keyword, str):
                    if keyword.startswith('/') or keyword.startswith('http'):
                        ...
                        if self.enable_amplification:
                            ...
                            if topic_data and isinstance(topic_data, dict):
                                topic_name = topic_data.get('name') or topic_data.get('title')
                                if topic_name:
                    ...
                    if 'entities' in keyword:
                        ...
                        for entity in keyword.get('entities', []):
                            if isinstance(entity, dict) and 'details' in entity:
                                ...
                                entity_id = details.get('uber_cat_id') or entity.get('id')
                        ...
                        for entity in parsed_entities:
                            if entity['code_length'] == 4:
                                ...
                                if parent_code in field_groups:
                                    field_groups[parent_code]['subfields'].append(entity)
                                else:
                        ...
                        for field_code, group_data in field_groups.items():
                            if group_data['subfields']:
                                ...
                                for subfield in group_data['subfields']:
                                    ...
                                    topics_to_process.append((subfield['name'], entity_data))
                                ...
                                topics_to_process.append((group_data['field_name'], entity_data))
                    else:
                ...
                for topic_name, topic_data in topics_to_process:
                    ...
                    if topic_data and isinstance(topic_data, dict) and topic_data.get('id'):
                    ...
                    try:
                        existing_blob = self.dest_container_client.get_blob_client(topic_blob_name)
                    ...
                    topic_blob_client = self.dest_container_client.get_blob_client(topic_blob_name)
    ...
    def process_all(self, max_files: Optional[int] = None) -> Dict[str, int]:
        ...
        """
        logger.info(f"Starting to extract topics from {self.source_container}/{self.source_prefix}")
        ...
        logger.info(f"Processing complete: {processed} processed, {failed} failed, {total_topics_extracted} topics extracted")
...
def main():
    ...
    try:
        ...
        logger.info(f"This run: {results['total_topics_extracted']} topic instances extracted")
