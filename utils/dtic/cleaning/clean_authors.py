...
class DTICAuthorExtractor:
    ...
    def extract_and_clean_author(
        ...
        author_entity = {
            "id": author_id,
            "name": author_name,
            "sources": [{"source": "dtic", "id": researcher_id}],
        ...
        try:
            ...
            for org in raw_work.get("organizations", []):
                if org.get("name") and org.get("org_id"):
                    org_id_mapping[org["name"]] = self.get_org_guid_from_grid(
                        org["org_id"]
                    )
            ...
            for author in authors:
                ...
                author_blob_client = self.dest_container_client.get_blob_client(
                    author_blob_name
                )
            ...
            logger.info(
                f"[OK] Processed {blob_name}: extracted {authors_found} authors"
            )
    ...
    def process_all(self, max_files: Optional[int] = None) -> Dict[str, int]:
        ...
        """
        logger.info(
            f"Starting to extract authors from {self.source_container}/{self.source_prefix}"
        )
        ...
        logger.info(
            f"Processing complete: {processed} processed, {failed} failed, {total_authors_extracted} authors extracted"
        )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"This run: {results['total_authors_extracted']} author instances extracted"
        )
