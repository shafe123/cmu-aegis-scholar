...
class DTICWorkExtractor:
    ...
    def __init__(
        ...
        logger.info("Initialized DTIC Work Extractor")
    ...
    def extract_and_clean_work(self, raw_work: Dict) -> Optional[Dict]:
        ...
        if not publication_id or not title:
            logger.warning("Skipping work without publication_id or title")
            return None
        ...
        work_entity = {
            "id": work_id,
            "title": title,
            "sources": [{"source": "dtic", "id": publication_id}],
            ...
            for keyword_group in keywords:
                if isinstance(keyword_group, dict):
                    ...
                    for entity in entities:
                        ...
                        if uber_cat_id:
                            topic_guid = self.get_topic_guid_from_uber_cat_id(
                                uber_cat_id
                            )
                            # DTIC doesn't provide scores, use 1.0 as default
                            topic_entry = {"topic_id": topic_guid, "score": 1.0}
    ...
    def process_blob(self, blob_name: str) -> int:
        ...
        try:
            ...
            # Upload cleaned work
            dest_blob_client = self.dest_container_client.get_blob_client(
                work_blob_name
            )
    ...
    def process_all(self, max_files: Optional[int] = None) -> Dict[str, int]:
        ...
        """
        logger.info(
            f"Starting to extract works from {self.source_container}/{self.source_prefix}"
        )
...
def main():
    ...
    parser.add_argument(
        "--source-container", default="raw", help="Source container name"
    )
    parser.add_argument(
        "--source-prefix", default="dtic/works/", help="Source blob prefix"
    )
    parser.add_argument(
        "--dest-container", default="clean", help="Destination container name"
    )
    parser.add_argument(
        "--dest-prefix", default="dtic/works/", help="Destination blob prefix"
    )
