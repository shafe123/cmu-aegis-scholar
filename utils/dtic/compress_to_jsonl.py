...
class JSONLCompressor:
    ...
    def compress_entity_type(self, 
        ...
        try:
            for i, file_path in enumerate(files):
                try:
                    ...
                    if current_chunk_file is None or current_chunk_size >= self.target_size_bytes:
                        # Close previous chunk if exists
                        if current_chunk_file is not None:
                            current_chunk_file.close()
                            actual_size = current_chunk_path.stat().st_size
                            total_size += actual_size
                            logger.info(f"  Completed chunk {chunk_num-1}: {files_in_chunk} files, "
                        ...
                        if dry_run:
                            ...
                            current_chunk_file = gzip.open(current_chunk_path, 'wb', compresslevel=6)
                            current_chunk_size = 0
                            files_in_chunk = 0
        ...
        logger.info(f"Completed {entity_type}: {files_processed} files -> "
    ...
    def compress_all(self, 
        ...
        logger.info(f"Target chunk size: {self.target_size_bytes / (1024*1024):.1f} MB compressed")
...
def main():
    ...
    parser.add_argument(
        ...
        help=f'Entity types to process (default: all)'
    )
    parser.add_argument(
        '--target-size',
        type=float,
