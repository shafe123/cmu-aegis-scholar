        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, "r", encoding="utf-8") as f:
                    state = json.load(f)
                    logger.info(
                        f"Loaded state: {len(state.get('amplified_files', []))} files amplified"
                    )
                    return state
...
class KeywordAmplifier:
    ...
    def fetch_keywords(self, keyword_url_path: str) -> Optional[List[str]]:
        ...
        try:
            ...
            if isinstance(keywords_data, list):
                ...
                for field in ["keywords", "fields", "subjects", "for", "data"]:
                    if field in keywords_data:
                        return (
                            keywords_data[field]
                            if isinstance(keywords_data[field], list)
                            else [keywords_data[field]]
                        )
            ...
            logger.warning(
                f"Failed to parse keywords JSON from {keyword_url_path}: {e}"
            )
            return None
    ...
    def amplify_publication(self, pub_data: Dict) -> Dict:
        ...
        for keyword in keywords:
            if isinstance(keyword, str) and (
                keyword.startswith("/") or keyword.startswith("http")
            ):
    ...
    def list_source_blobs(self) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(blob_list)} JSON blobs with prefix '{self.source_prefix}'"
            )
            return blob_list
...
def main():
    ...
    parser.add_argument(
        "--container", default="raw", help="Blob container name (default: raw)"
    )
    parser.add_argument(
        "--source-prefix",
        default="dtic/works/",
    ...
    if not connection_string:
        ...
        logger.error(
            "Use --connection-string or set AZURE_STORAGE_CONNECTION_STRING environment variable"
        )
        return 1
