...
class DTICAnalyzer:
    ...
    def _load_data(self):
        ...
        print(f"Loaded {len(self.publications)} publications from {len(json_files)} files")
    ...
    def summary_stats(self):
        ...
        print(f"Publications with abstracts: {with_abstract} ({with_abstract/len(self.publications)*100:.1f}%)")
        print(f"Publications with DOI: {with_doi} ({with_doi/len(self.publications)*100:.1f}%)")
        print(f"Publications with keywords: {with_keywords} ({with_keywords/len(self.publications)*100:.1f}%)")
        print(f"Publications with citation counts: {with_citations} ({with_citations/len(self.publications)*100:.1f}%)")
    ...
    def citation_stats(self):
        ...
        citations = [p.get('citations_count', 0) for p in self.publications if p.get('citations_count') is not None]
        ...
        cited_pubs = [(p.get('citations_count', 0), p.get('title', 'Unknown'), p.get('publication_id', '')) 
                      for p in self.publications if p.get('citations_count')]
    ...
    def export_csv(self, output_file: str = "dtic_publications.csv"):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            ...
            fieldnames = ['publication_id', 'title', 'abstract', 'publication_date', 
                         'doi', 'document_type', 'citations_count', 'url', 
                         'author_names', 'organization_names', 'keywords']
...
def main():
    ...
    parser.add_argument('--input-dir', '-i', default='dtic_publications',
                      help='Input directory with JSON files (default: dtic_publications)')
    parser.add_argument('--export-csv', '-e', metavar='FILE',
