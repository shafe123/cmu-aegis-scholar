...
class BlobDownloader:
    ...
    def __init__(
        ...
        if connection_string:
            ...
            self.blob_service_client = BlobServiceClient(
                account_url=account_url_with_sas
            )
    ...
    def download_blob(
        self, blob_name: str, output_dir: Path, dry_run: bool = False
    ) -> tuple[str, bool]:
        """
    ...
    def download_all(
        ...
        if dry_run:
            logger.info(
                f"[DRY RUN] Would download {len(blob_names)} blobs to {output_dir}"
            )
            ...
            logger.info(
                f"Starting download of {len(blob_names)} blobs using {max_workers} workers"
            )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_blob = {
                    executor.submit(
                        self.download_blob, blob_name, output_dir
                    ): blob_name
                    for blob_name in blob_names
                }
                ...
                for future in as_completed(future_to_blob):
                    ...
                    if total_processed % 10 == 0 or total_processed == len(blob_names):
                        logger.info(
                            f"Progress: {total_processed}/{len(blob_names)} "
...
def main():
    ...
    auth_group.add_argument(
        "--sas-token", help="Azure Storage SAS token (including the ? prefix)"
    )
