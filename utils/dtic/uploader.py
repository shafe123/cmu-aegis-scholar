        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    state = json.load(f)
                    logger.info(f"Loaded state: {len(state.get('uploaded_files', []))} files uploaded")
                    return state
...
class AzureBlobUploader:
    ...
    def scan_and_upload(self) -> tuple[int, int]:
        ...
        if not self.publications_dir.exists():
            logger.error(f"Publications directory does not exist: {self.publications_dir}")
            return 0, 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Azure Blob Storage Uploader for DTIC Publications")
    ...
    parser.add_argument('--container', '-n', default='raw',
                      help='Container name (default: raw)')
    parser.add_argument('--publications-dir', '-d', default='dtic_publications',
                      help='Publications directory (default: dtic_publications)')
    parser.add_argument('--state', '-s', default='upload_state.json',
                      help='State file path (default: upload_state.json)')
    parser.add_argument('--blob-prefix', '-p', default='dtic/works/',
    ...
    if not connection_string:
        parser.error('Connection string is required. Provide via --connection-string or set AZURE_STORAGE_CONNECTION_STRING environment variable.')
