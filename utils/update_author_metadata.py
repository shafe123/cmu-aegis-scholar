...
class AuthorMetadataUpdater:
    ...
    def build_author_lookup(self) -> dict[str, dict]:
        ...
        if not author_files:
            logger.error(
                f"No author files found matching pattern '{pattern}' in {self.data_dir}"
            )
            return lookup
        ...
        logger.info(
            f"Built lookup table with {total_authors} authors ({len(lookup)} unique)"
        )
        return lookup
    ...
    def connect_to_milvus(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded collection '{self.collection_name}' with {num_entities} entities"
            )
    ...
    def _ensure_schema_updated(self):
        ...
        try:
            ...
            if "citation_count" not in field_names:
                logger.warning(
                    "citation_count field not found in schema - migration required"
                )

                if self.dry_run:
                    logger.warning(
                        "[DRY RUN] Migration skipped - will simulate updates without citation_count field"
                    )
                    logger.warning(
                        "[DRY RUN] Run without --dry-run to perform actual schema migration"
                    )
                    return

                logger.info(
                    "Migrating collection schema to add citation_count field..."
                )
                ...
                if "citation_count" in new_field_names:
                    logger.info(
                        "✓ Schema migration successful - citation_count field added"
                    )
                else:
                    raise RuntimeError(
                        "Schema migration failed - citation_count field still missing"
                    )
            else:
    ...
    def _migrate_schema(self):
        ...
        try:
            ...
            if utility.has_collection(temp_collection_name):
                logger.warning(
                    f"Found existing temp collection '{temp_collection_name}' - cleaning up..."
                )
                utility.drop_collection(temp_collection_name)

            # Step 1: Create new collection with updated schema
            logger.info(
                f"Creating temporary collection '{temp_collection_name}' with updated schema..."
            )
            ...
            fields = [
                ...
                FieldSchema(
                    name="author_name", dtype=DataType.VARCHAR, max_length=1024
                ),
            ...
            logger.info(
                f"✓ Created temp collection with schema: {[f.name for f in fields]}"
            )
            ...
            logger.info(
                f"✓ New collection loaded with {temp_collection.num_entities} entities"
            )
            ...
            logger.info(
                f"Renaming '{temp_collection_name}' to '{self.collection_name}'..."
            )
    ...
    def get_all_vector_entries(self) -> list[dict]:
        ...
        try:
            ...
            if not has_citation_count:
                logger.info(
                    "Adding citation_count=0 to all entries (field not in schema)"
                )
        ...
        try:
            ...
            if has_citation_count:
                entity_data = [
                    ...
                    [
                        entry["author_name"] for entry in entries_to_update
                    ],  # author_name
                    [
                        entry["embedding"] for entry in entries_to_update
                    ],  # embedding (preserved)
                    [
                        entry["num_abstracts"] for entry in entries_to_update
                    ],  # num_abstracts
                    [
                        entry["citation_count"] for entry in entries_to_update
                    ],  # citation_count
                ]
            else:
                entity_data = [
            ...
            logger.info(
                f"Successfully updated batch of {len(entries_to_update)} entries"
            )
            return len(entries_to_update)
        ...
        try:
            ...
            for entry in all_entries:
                ...
                if needs_update:
                    ...
                    if len(entries_to_update) % 100 == 0:
                        logger.info(
                            f"  Found {len(entries_to_update)} entries to update so far..."
                        )
            ...
            if entries_to_update:
                ...
                for i in range(0, len(entries_to_update), self.batch_size):
                    ...
                    logger.info(
                        f"  Progress: {min(i + self.batch_size, len(entries_to_update))}/{len(entries_to_update)}"
                    )

                logger.info(f"Successfully updated {self.stats['updated']} entries")
            else:
                logger.info(
                    "No entries need updating - all metadata is already correct!"
                )
            ...
            logger.info(
                f"Entries needing citation updates: {self.stats['citation_updates']}"
            )
