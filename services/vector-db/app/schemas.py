...
class VectorSearchRequest(BaseModel):
    ...
    query_vector: List[float] = Field(
        ...
        json_schema_extra={"example": {"query_vector": 
...
class CreateAuthorVectorRequest(BaseModel):
    ...
    embedding: List[float] = Field(
        ...
        json_schema_extra={"example": {"embedding": 
                                       [0.025, -0.134, 0.078]}},  # 384-dim vector
    )
    model_name: Optional[str] = Field(
        None,
...
class PaginationMetadata(BaseModel):
    ...
    returned: int = Field(..., json_schema_extra={"example": 5})
    has_more: bool = Field(..., json_schema_extra={"example": True})
...
class CreateAuthorEmbeddingResponse(BaseModel):
    ...
    author_name: str = Field(..., json_schema_extra={"example": "Dr. Sarah Chen"})
    embedding_dim: int = Field(..., json_schema_extra={"example": 384})
    num_abstracts_processed: int = Field(..., json_schema_extra={"example": 2})
    collection_name: str = Field(..., json_schema_extra={"example": "aegis_vectors"})
