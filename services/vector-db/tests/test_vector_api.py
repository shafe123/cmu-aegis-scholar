...
def test_text_search_validation():
    ...
    response = client.post("/search/text", json={"query_text": ""})
    assert response.status_code == 422  # Validation error
...
def test_text_search_with_valid_input():
    ...
    response = client.post(
        "/search/text",
        json={"query_text": "machine learning applications in healthcare", "limit": 5},
    )
...
def test_text_search_with_invalid_model():
    ...
    response = client.post(
        "/search/text",
        json={"query_text": "test query", "model_name": "nonexistent/model"},
    )
...
def test_create_author_embedding_validation():
    ...
    response = client.post(
        "/authors/embeddings",
        json={"author_name": "John Doe", "abstracts": ["Some abstract text"]},
    )
    assert response.status_code == 422  # Validation error
    ...
    response = client.post(
        "/authors/embeddings",
        json={"author_id": "123", "author_name": "John Doe", "abstracts": []},
    )
    assert response.status_code == 422  # Validation error
...
def test_create_author_vector_validation():
    ...
    response = client.post(
        "/authors/vector",
        json={"author_id": "123", "author_name": "John Doe", "embedding": []},
    )
